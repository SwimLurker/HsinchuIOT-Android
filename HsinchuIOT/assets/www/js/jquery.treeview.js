;(function($){
	$.extend($.fn,{
		...
		,applyClasses:function(settings,toggler){
			...
			this.find("div."+CLASSES.hitarea).mousedown(toggler);
			var ul=this;
			this.find("a").click(function(){
				if(!this.root)this.root=$(this).parentsUntil('ul.treeview').parent();
				$('a.selected',this.root).removeClass('selected');
				$(this).addClass('selected');
				if($(this.parentNode).hasClass(CLASSES.expandable))$(this.previousSibling).mousedown();
			});
		}
		,treeview:function(settings){
			settings=$.extend({cookieId:"treeview"},settings);
			if(settings.add){
				var branches=settings.add;$(branches).prev().removeClass(CLASSES.last).removeClass(CLASSES.lastCollapsable).removeClass(CLASSES.lastExpandable).find(">.hitarea").removeClass(CLASSES.lastCollapsableHitarea).removeClass(CLASSES.lastExpandableHitarea);
