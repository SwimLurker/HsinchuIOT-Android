...
public class SuperUserSiteDetailActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		currentSite = (Site) getIntent().getSerializableExtra(SuperUserMainActivity.SELECTED_SITE);
		warningThreshold = (IOTMonitorThreshold) getIntent()
				.getSerializableExtra(SuperUserMainActivity.WARNING_THRESHOLD);
		alarmThreshold = (IOTMonitorThreshold) getIntent().getSerializableExtra(SuperUserMainActivity.ALARM_THRESHOLD);
		...
		if (currentSite != null) {
			sendQueryReportRequest(currentSite.getDevice().getDeviceID(), ReportUtil.getServerTimeString(co2From),
					ReportUtil.getServerTimeString(co2To), new CO2ReportDataHandler());

			sendQueryReportRequest(currentSite.getDevice().getDeviceID(),
					ReportUtil.getServerTimeString(temperatureFrom), ReportUtil.getServerTimeString(temperatureTo),
					new TemperatureReportDataHandler());

			sendQueryReportRequest(currentSite.getDevice().getDeviceID(), ReportUtil.getServerTimeString(humidityFrom),
					ReportUtil.getServerTimeString(humidityTo), new HumidityReportDataHandler());
		}
	}
	...
	private void initViews() {
		...
		refreshScrollView.setOnRefreshListener(new OnRefreshListener<ScrollView>() {
			...
			public void onRefresh(PullToRefreshBase<ScrollView> refreshView) {
				if (currentSite != null) {
					sendQueryReportRequest(currentSite.getDevice().getDeviceID(),
							ReportUtil.getServerTimeString(co2From), ReportUtil.getServerTimeString(co2To),
							new CO2ReportDataHandler());

					sendQueryReportRequest(currentSite.getDevice().getDeviceID(),
							ReportUtil.getServerTimeString(temperatureFrom),
							ReportUtil.getServerTimeString(temperatureTo), new TemperatureReportDataHandler());

					sendQueryReportRequest(currentSite.getDevice().getDeviceID(),
							ReportUtil.getServerTimeString(humidityFrom), ReportUtil.getServerTimeString(humidityTo),
	...
	private boolean showReport(int timePeriod, Calendar from, Calendar to, TextView tvTitle, IReportDataHandler handler,
		...
		if (currentSite != null) {
			sendQueryReportRequest(currentSite.getDevice().getDeviceID(), ReportUtil.getServerTimeString(from),
