...
public class SuperUserSiteDetailActivity extends BaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (currentSite != null) {
			sendQueryReportRequest(currentSite.getDevice().getDeviceID(),
					ReportUtil.getServerTimeString(co2From),
					ReportUtil.getServerTimeString(co2To),
					new CO2ReportDataHandler());

			sendQueryReportRequest(currentSite.getDevice().getDeviceID(),
					ReportUtil.getServerTimeString(temperatureFrom),
					ReportUtil.getServerTimeString(temperatureTo),
					new TemperatureReportDataHandler());

			sendQueryReportRequest(currentSite.getDevice().getDeviceID(),
					ReportUtil.getServerTimeString(humidityFrom),
					ReportUtil.getServerTimeString(humidityTo),
					new HumidityReportDataHandler());
		}
	}

	private void initViews() {
		ivSiteImage = (ImageView) findViewById(R.id.iv_site_image);
		String uri = "file://" + Constants.ImageLoader.IMAGE_ENGINE_CACHE + "/"
				+ currentSite.getSiteImageFilename();
		...
		tvCO2Title = (TextView) findViewById(R.id.tv_co2_title);
		tvCO2Title.setText(resources.getString(R.string.co2) + ": "
				+ ReportUtil.get8HoursTimePeriodString(co2From, co2To));

		tvTemperatureTitle = (TextView) findViewById(R.id.tv_temperature_title);
		tvTemperatureTitle.setText(resources.getString(R.string.temperature)
				+ ": "
				+ ReportUtil.get1HourTimePeriodString(temperatureFrom,
						temperatureTo));

		tvHumidityTitle = (TextView) findViewById(R.id.tv_humidity_title);
		tvHumidityTitle
				.setText(resources.getString(R.string.humidity)
						+ ": "
						+ ReportUtil.get1HourTimePeriodString(humidityFrom,
								humidityTo));

		tvCO2AverageValue = (TextView) findViewById(R.id.tv_co2_averagevalue);
		tvCO2AverageValue.setText("");
		tvCO2MaxValue = (TextView) findViewById(R.id.tv_co2_maxvalue);
		tvCO2MaxValue.setText("");
		tvCO2MinValue = (TextView) findViewById(R.id.tv_co2_minvalue);
		tvCO2MinValue.setText("");

		tvTemperatureAverageValue = (TextView) findViewById(R.id.tv_temperature_averagevalue);
		tvTemperatureAverageValue.setText("");
		tvTemperatureMaxValue = (TextView) findViewById(R.id.tv_temperature_maxvalue);
		tvTemperatureMaxValue.setText("");
		tvTemperatureMinValue = (TextView) findViewById(R.id.tv_temperature_minvalue);
		tvTemperatureMinValue.setText("");

		tvHumidityAverageValue = (TextView) findViewById(R.id.tv_humidity_averagevalue);
		tvHumidityAverageValue.setText("");
		tvHumidityMaxValue = (TextView) findViewById(R.id.tv_humidity_maxvalue);
		tvHumidityMaxValue.setText("");
		tvHumidityMinValue = (TextView) findViewById(R.id.tv_humidity_minvalue);
		tvHumidityMinValue.setText("");

		vfCO2 = (ViewFlipper) findViewById(R.id.vf_co2);
		...
		vfTemperature = (ViewFlipper) findViewById(R.id.vf_temperature);
		vfTemperature.setOnTouchListener(new MyViewFlipperOnTouchListener(
				vfTemperature) {
		...
		vfHumidity = (ViewFlipper) findViewById(R.id.vf_humidity);
		vfHumidity.setOnTouchListener(new MyViewFlipperOnTouchListener(
				vfHumidity) {
		...
		btnCO2Pre = (ImageButton)findViewById(R.id.btn_co2_pre);
		btnCO2Pre.setOnClickListener(new OnClickListener(){

			@Override
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				vfCO2.setInAnimation(AnimationUtils.loadAnimation(
						SuperUserSiteDetailActivity.this,
						R.anim.slide_in_right));
				vfCO2.setOutAnimation(AnimationUtils.loadAnimation(
						SuperUserSiteDetailActivity.this,
		...
		btnCO2Next = (ImageButton)findViewById(R.id.btn_co2_next);
		...
		btnTemperaturePre = (ImageButton)findViewById(R.id.btn_temperature_pre);
		btnTemperaturePre.setOnClickListener(new OnClickListener(){

			@Override
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				vfTemperature.setInAnimation(AnimationUtils.loadAnimation(
						SuperUserSiteDetailActivity.this,
						R.anim.slide_in_right));
				vfTemperature.setOutAnimation(AnimationUtils.loadAnimation(
						SuperUserSiteDetailActivity.this,
						R.anim.slide_out_left));
		...
		btnTemperatureNext = (ImageButton)findViewById(R.id.btn_temperature_next);
		...
		btnHumidityPre = (ImageButton)findViewById(R.id.btn_humidity_pre);
		btnHumidityPre.setOnClickListener(new OnClickListener(){

			@Override
			public void onClick(View arg0) {
				// TODO Auto-generated method stub
				vfHumidity.setInAnimation(AnimationUtils.loadAnimation(
						SuperUserSiteDetailActivity.this,
						R.anim.slide_in_right));
				vfHumidity.setOutAnimation(AnimationUtils.loadAnimation(
						SuperUserSiteDetailActivity.this,
						R.anim.slide_out_left));
		...
		btnHumidityNext = (ImageButton)findViewById(R.id.btn_humidity_next);
	...
	private void updateUI() {
		...
		tvCO2MaxValue.setText(Integer.toString(Math.round(maxValueCO2))
				+ " ppm");
		tvCO2MinValue.setText(Integer.toString(Math.round(minValueCO2))
				+ " ppm");

		tvTemperatureAverageValue.setText(Float
				.toString(averageValueTemperature) + " ℃");
		...
		tvHumidityMaxValue.setText(Integer.toString(Math
				.round(maxValueHumidity)) + " %");
		tvHumidityMinValue.setText(Integer.toString(Math
				.round(minValueHumidity)) + " %");
	}

	private boolean showNextCO2() {
		return showReport(8, co2From, co2To, tvCO2Title,
				new CO2ReportDataHandler(), resources.getString(R.string.co2));
	}
	...
	private boolean showNextTemperature() {
		return showReport(1, temperatureFrom, temperatureTo,
				tvTemperatureTitle, new TemperatureReportDataHandler(),
	...
	private boolean showNextHumidity() {
		return showReport(1, humidityFrom, humidityTo, tvHumidityTitle,
				new HumidityReportDataHandler(),
	...
	private boolean showReport(int timePeriod, Calendar from, Calendar to,
			TextView tvTitle, IReportDataHandler handler, String title) {
		...
		if (newFrom.after(now)) {
			Toast.makeText(this, "No data for this time period",
		...
		if (currentSite != null) {
			sendQueryReportRequest(currentSite.getDevice().getDeviceID(),
					ReportUtil.getServerTimeString(from),
					ReportUtil.getServerTimeString(to), handler);
		}
		return true;

	}

	private void sendQueryReportRequest(String deviceID, String from,
			String to, IReportDataHandler handler) {
		...
		GetReportDataListener l = new GetReportDataListener(handler, deviceID,
				from, to);

		ServiceContainer.getInstance().getHttpHandler().doRequest(request, l);
	}

	private abstract class MyViewFlipperOnTouchListener implements
			OnTouchListener {
	...
	private class GetReportDataListener implements
			RequestListener<IOTReportData> {
		...
		public GetReportDataListener(IReportDataHandler handler,
	...
	private class TemperatureReportDataHandler implements IReportDataHandler {
		...
		public void handle(IOTReportData reportData) {
			...
			timeOfMaxValueTemperature = reportData
					.getTimeOfMaxValueTemperature();
			minValueTemperature = reportData.getMinValueTemperature();
			timeOfMinValueTemperature = reportData
					.getTimeOfMinValueTemperature();
		}
